...
struct Coordinate
{
	...
	void MirrorX() { x = -x; }
	void MirrorY() { y = -y; }
	Coordinate GetMirror() const { return { -x, -y }; }
	Coordinate GetMirrorX() const { return { -x, y }; }
	Coordinate GetMirrorY() const { return { x, -y }; }
	...
	{
		return { (int)(coord.x * k), (int)(coord.y * k) };
	};
...
struct Vector
{
	...
	void MirrorX() { x = -x; }
	void MirrorY() { y = -y; }
	Vector GetMirror() const { return { -x, -y }; }
	Vector GetMirrorX() const { return { -x, y }; }
	Vector GetMirrorY() const { return { x, -y }; }
...
struct Rect
{
	...
	void SetPos(const Coordinate& _pos) { pos = _pos; }
	...
	Coordinate GetCenter() const { return Coordinate(x + (width >> 1), y + (height >> 1)); }
	...
	*/
	...
	{
		return Rect((int)(pos.x * k), (int)(pos.y * k), (int)(width * k), (int)(height * k));
	}

	bool operator==(const Rect& rect)
	{
		return (pos == rect.pos) && (width == rect.width) && (height == rect.height);
	}
};
